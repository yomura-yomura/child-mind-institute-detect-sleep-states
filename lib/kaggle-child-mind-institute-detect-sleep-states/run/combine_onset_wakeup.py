...
path_dict = {
    "onset": predicted_dir_path / "ranchantan" / "exp050-transformer-decoder_retry_resume" / "train",
...
for i_fold in range(5):
    onset_stems, wakeup_stems = [
        set(p.stem for p in (path_dict[k] / f"fold_{i_fold}").glob("*.npz")) for k in ["onset", "wakeup"]
    ]
    ...
    for series_id in series_ids:
        ...
        wakeup_preds = np.load(path_dict["wakeup"] / f"fold_{i_fold}" / f"{series_id}.npz")["arr_0"]
