...
def preprocess_for_psuedo_labels(dict_data,result_path:str,score_th:float = 0.5) -> None:
    """submitファイルのscore_th以上のラベルを抽出
    psuedo_labeling v1で使用
    """
    ...
    pd.concat(list_df).to_csv(f"{result_path}/psuedo_label_{score_th}.csv",index = None)
    return 
...
if __name__ == "__main__":
    ...
    # 保存先を指定
    result_path = Path(__file__).parent.parent / "psuedo_label"

    result_path.mkdir(exist_ok=True,parents = True)
    preprocess_for_psuedo_labels(dict_data=dict_data,result_path = result_path)
