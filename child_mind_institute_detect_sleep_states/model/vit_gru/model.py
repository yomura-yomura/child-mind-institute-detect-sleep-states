...
class VitGru(nn.Module):
    def __init__(
        ...
        self.ch = feature_num  # feature_num
        # self.seg = seg
        self.seq = duration
        self.patch = patch
        ...
        # )
        self.xformer = VisionTransformer(
            img_size=(duration // self.patch, 1),
            patch_size=(1, 1),
            in_chans=dims,
            num_classes=0,
            global_pool="",
        ...
        if xformer_layers > 0:  # and not rel_pos:
            ...
            self.xformer.pos_embed.data[:] += 0.02 * getPositionalEncoding(
                duration // self.patch, dims, 1000
            ).unsqueeze(0)
        if xformer_layers > 0:
            for i, b in enumerate(self.xformer.blocks):
        ...
        self.embed = nn.Sequential(
            PatchEmbed(
                img_size=(self.seq // self.patch, self.ch),
                patch_size=(1, self.ch),
                in_chans=self.patch,
                embed_dim=dims,
        ...
        self.last_layer = nn.Linear(self.seq // patch, self.out_size)

    def vit_gru(self, x):
        patch = self.patch
        # must be reshape to x: (batch_size, duration,patch,in_channels)
        # x: (batch_size, in_channels, time_steps)
        x = x.reshape(
            x.shape[0], x.shape[1], self.seq // patch, patch
        )  # x: (batch_size, in_channels, duration // patch, patch)
        x = x.permute(0, 2, 3, 1)  # x: (batch_size, duration // patch,patch,in_channels)

        attn = x.reshape(x.shape[0], -1, patch * self.ch)  # x: (batch_size, duration // patch, in_channels)
        attn = 1 * (attn.std(-1) > 1e-5)  # x:(batch_size, duration // patch)
        x = x.permute(0, 2, 1, 3)  # x: (batch_size, patch, duration // patch,in_channels)

        if len(x.shape) == 3:
            x = x.unsqueeze(1)
        x = self.embed(x)  # x: (batch_size, duration//patch, dim)
        x = self.patch_act(x)  # x: (batch_size, duration//patch, dim)
        # if self.seg:
        #    x = x.mean(-1).permute(0, 2, 1)
        if self.pre_norm:
            x = self.norm(x)  # x: (batch_size, duration//patch, dim)
        x = x * attn.unsqueeze(-1)  # x: (batch_size, duration//patch, dim)
        x = self.xformer(x)  # x: (batch_size, duration//patch, dim)
        x = x * attn.unsqueeze(-1)  # x: (batch_size, duration//patch, dim)
        if self.rnn is not None:
            x = self.dropout(x)  # x: (batch_size, duration//patch, dim)
            x = self.patch_dropout(x)  # x: (batch_size, duration//patch, dim)
        x = (
            self.rnn(x, self.h0.unsqueeze(1).repeat(1, x.shape[0], 1) if torch.is_tensor(self.h0) else self.h0)[0]
            if self.rnn is not None
            else x
        )  # x: (batch_size, duration//patch, dim*4) only use x

        x = x.transpose(1, 2)  # x: (batch_size, dim*4, duration//patch)
        return x  # x: (batch_size, 1, dim*4, duration//patch)

    def forward(self, x):
        x = self.vit_gru(x)
        x = self.last_layer(x)
        x = x.unsqueeze(1)  # x: (batch_size, 1, dim*4, output_size)
        return x
