...
def split_unlabeled_intervals(df_event:pd.DataFrame,df_series:pd.DataFrame,add_interval:int = 12*60*12,min_col:int = 12*12*60*2):
    """
...
def find_del_intervals(df:pd.DataFrame,max_step:int,col_night:str = "night",col_step:str = "step")->list[tuple[int,int]]:
